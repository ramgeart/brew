...
The script installs Homebrew to its default prefix (`~/.brew` in the user's home directory) so that [you don't need *sudo* after Homebrew's initial installation](FAQ.md#why-does-homebrew-say-sudo-is-bad) when you `brew install`. **Note:** Installing to `~/.brew` means pre-built binary packages (bottles) are not available and formulae will be built from source, which takes longer and may be less reliable. It is a careful script; it can be run even if you have stuff installed in the preferred prefix already. It tells you exactly what it will do before it does it too. You have to confirm everything it will do before it starts.

The macOS `.pkg` installer also installs Homebrew to its default prefix (`~/.brew` in the user's home directory) for the same reasons as above. **Note:** This prefix does not support pre-built binary packages (bottles), so all formulae must be built from source. It's available on [Homebrew/brew's latest GitHub release](https://github.com/Homebrew/brew/releases/latest). To specify an alternate install user, like in situations where the package is installed at the login window before a user has logged in, write a property list file to `/var/tmp/.homebrew_pkg_user.plist` with the value `HOMEBREW_PKG_USER`. For example, `defaults write /var/tmp/.homebrew_pkg_user HOMEBREW_PKG_USER penny`. The file and user must exist prior to install.
